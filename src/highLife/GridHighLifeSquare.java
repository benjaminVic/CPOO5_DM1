...
public class GridHighLifeSquare implements Grid<State, SquareGridNbh, CellHighLifeSquare>{

	private CellHighLifeSquare[][] tableau;
	 ...
	 */
	public GridHighLifeSquare(int rows, int colums, boolean handlesDiagonals){
		int enumElems;
		...
		tableau = new CellHighLifeSquare[rows][colums];
		for (int i = 0;i<rows;i++){
			for (int j = 0; j<colums ; j++){
				tableau[i][j] = new CellHighLifeSquare(enumElems);
			}
		}
		neighborConstructor();
	}
	 ...
	 */
	public GridHighLifeSquare(String preset, boolean handlesDiagonals){
		this.handlesDiagonals = handlesDiagonals;
		switch (preset){
		case ("a") :
			this.rows = 15;
		...
		case ("b") :
			...
			for (int i = 0; i < rows; i++) {
				for (int j = 0; j < colums; j++) {
					tableau[i][j] = new CellHighLifeSquare(enumElems2);
				}
			}
	 ...
	 */
	public CellHighLifeSquare getCell(int row, int colum){
